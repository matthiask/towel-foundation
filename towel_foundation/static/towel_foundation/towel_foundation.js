...
;(function ($) {
  ...
  onForm.push(function (elem, $) {
    if ($.fn.chosen) elem.find("select").not(".plain").chosen()
    if ($.fn.datePicker) {
        ...
        var $form = $(this),
          handleResponse = function (data) {
            ...
            if (typeof data == "string") {
              ...
              updateLive(data)
              if (!("!keep" in data)) $elem.foundation("reveal", "close")
            }
          }
        ...
        modal.on("close", function (event) {
          ...
          if (window.modalLoad.locked) {
            if (
              !confirm(
                gettext(
                  "You have unsaved changes. Do you really want to throw them away?"
                )
              )
            ) {
              return false
